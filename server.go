...
//
func (headerMaster *HeaderMaster) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	log.Debug("HeadersMaster ServeHTTP")
	if headerMaster.rule != "" {
		r.Host = headerMaster.rule
	}
	log.Debug("HeadersMaster ServeHTTP - End")
			...
			for _, entryPointName := range frontend.EntryPoints {
				...
				if entryPoint.Redirect != nil {
					...
					if backends[frontend.Backend] == nil {
						...
						var negroni = negroni.New()
						negroni.UseHandler(&HeaderMaster{frontend.RewriteHostHeader})
