...
func (provider *Docker) loadDockerConfig(containersInspected []dockertypes.ContainerJSON) *types.Configuration {
	var DockerFuncMap = template.FuncMap{
		...
		"replace":              replace,
		"getRewriteHostHeader": provider.getRewriteHostHeader,
	}
...
func (provider *Docker) getRewriteHostHeader(container dockertypes.ContainerJSON) string {
	if rewriteHost, err := getLabel(container, "traefik.backend.rewrite.host"); err == nil {
		return rewriteHost
	}
	return ""
}
